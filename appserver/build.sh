...
if [ $resources_exists = true ]; then
  ...
  docker build --no-cache -t juliansantosinfo/totvs_appserver:release-2510.build-24.3.1.1.dbapi-24.1.1.0 .
else
...
if [ -d ./totvs ]; then
  ...
  if [ "$resposta" = "s" ] || [ "$resposta" = "S" ]; then
    ...
    # Comprime o arquivo em partes de 50MB
    tar -czvf - totvs | split -b 50m - totvs.tar.gz

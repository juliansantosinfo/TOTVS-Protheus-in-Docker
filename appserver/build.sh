...
readonly REQUIRED_DIR_NAME="appserver"
...
readonly DOCKER_IMAGE_TAG="${APPSERVER_VERSION}"
readonly DOCKER_TAG="${DOCKER_USER}/${APPSERVER_IMAGE_NAME}:${DOCKER_IMAGE_TAG}"
...
while [ ! -f "./totvs/protheus.tar.gz" ] || [ ! -f "./totvs/protheus_data.tar.gz" ]; do
    echo "⏳ Arquivos não encontrados. Executando setup..."
...
if [  -f "./totvs/protheus.tar.gz" ]; then
    echo "✅ Arquivo 'protheus.tar.gz' localizado."
else
    echo "❌ Erro: Arquivo protheus.tar.gz não encontrado."
    exit 1
fi

if [ -f "./totvs/protheus_data.tar.gz" ]; then
    echo "✅ Arquivo 'protheus_data.tar.gz' localizado."
else
    echo "❌ Erro: Arquivo protheus_data.tar.gz não encontrado."
    exit 1
fi

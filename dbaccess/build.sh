...
readonly REQUIRED_DIR_NAME="dbaccess"
...
readonly DOCKER_IMAGE_TAG="${DBACCESS_VERSION}"
readonly DOCKER_TAG="${DOCKER_USER}/${DBACCESS_IMAGE_NAME}:${DOCKER_IMAGE_TAG}"
...
if [  -d "./totvs/dbaccess" ]; then
    echo "✅ Diretório 'dbaccess' localizado."
else
    echo "❌ Erro: Diretório 'dbaccess' não encontrado."
    exit 1
fi

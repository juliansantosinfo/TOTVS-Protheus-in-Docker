...
# DESCRIÇÃO: Responsável por realizar o build da imagem Docker para o serviço Oracle
...
readonly DOCKER_IMAGE_TAG="${ORACLE_VERSION}"
...
# 2. Verifica se o subdiretório 'oracle' existe no local atual.
...
echo "🔍 Verificando o arquivo '${DATABASE_PATH}'..."
...
if [ -f "$DATABASE_PATH" ]; then
    echo "✅ Arquivo '${DATABASE_FILE}' localizado."
else
    echo "❌ Erro: Arquivo '${DATABASE_FILE}' não encontrado."
    exit 1
fi
